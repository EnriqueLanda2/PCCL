...
function RumboSeal({ size = 34 }: Readonly<{ size?: number }>) {
  return (
    <span
      className="inline-flex items-center justify-center rounded-[0.625rem] text-white flex-shrink-0"
    ...
    <div className={cn('holo-scene', className)}>
      <div ref={tiltRef} className="holo-tilt" onPointerMove={handleMove} onPointerLeave={handleLeave}>
        ...
        >
          ...
          <div className="holo-face absolute inset-0 rounded-[1.25rem] border-2 border-[var(--green-500)] bg-white shadow-[0_24px_50px_rgba(23,50,77,0.10)] overflow-hidden flex flex-col px-7 py-6">
            {/* Badge estado */}
            <span
              className="absolute top-4 right-4 rounded-full px-3 py-1 text-[0.6875rem] font-bold border"
            ...
            <div className="flex flex-col items-center gap-1.5 mt-1">
              <div className="flex items-center gap-2.5">
                <RumboSeal />
                <span className="text-[1.375rem] font-extrabold tracking-[0.08em] text-[var(--ink)]">RUMBO</span>
              </div>
              <span className="text-[0.5625rem] font-bold uppercase tracking-[0.3em] text-[var(--ink-muted)]">
            ...
            <div className="flex flex-col items-center text-center flex-1 justify-center gap-1 py-2">
              <span className="text-[0.8125rem] font-bold uppercase tracking-[0.22em] text-[var(--ink)]">
                Certificado de finalización
              </span>
              <span className="mt-2 text-[0.7813rem] text-[var(--ink-muted)]">Se otorga el presente a</span>
              <span className="text-[clamp(20px,2.2vw,28px)] font-extrabold tracking-tight text-[var(--ink)] leading-tight">
                {data.studentName}
              </span>
              <span className="text-[0.7813rem] text-[var(--ink-muted)]">por completar satisfactoriamente el curso</span>
              <span className="text-[0.9688rem] font-bold text-[var(--green-600)]">{data.courseTitle}</span>
            </div>

            {/* Pie: fecha · sello · instructor */}
            <div className="flex items-end justify-between gap-3">
              <div className="flex flex-col gap-0.5 min-w-0">
                <span className="text-[0.625rem] text-[var(--ink-muted)]">Fecha de emisión</span>
                <span className="text-[0.7813rem] font-bold text-[var(--ink)]">{formatDate(data.issuedAt)}</span>
              </div>
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <span className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-[var(--green-500)] text-[var(--green-600)] relative">
                  ...
                  <span className="absolute -bottom-1 rounded-full bg-white px-1 text-[0.3438rem] font-bold uppercase tracking-[0.14em] text-[var(--green-600)]">
                    Verificado
                  </span>
                </span>
              </div>
              ...
              <div className="flex flex-col items-center gap-0.5 min-w-0">
                <span className="text-[0.8125rem] font-bold text-[var(--ink)] border-t border-[var(--ink)] pt-1 px-2 truncate max-w-[8.75rem]">
                  {data.instructorName ?? 'Instructor'}
                </span>
                <span className="text-[0.625rem] text-[var(--ink-muted)]">Instructor</span>
              </div>
            </div>

            {/* Folio */}
            <div className="mt-4 border-t border-dashed border-[#DCE7D4] pt-2.5 text-center">
              <span className="font-mono text-[0.6563rem] tracking-[0.12em] text-[var(--ink-muted)]">
          ...
          {/* ══════════ REVERSO — datos + QR ══════════ */}
          <div className="holo-face holo-face-back rounded-[1.25rem] border-2 border-[var(--green-500)] overflow-hidden flex flex-col px-7 py-6 text-white"
            style={{ background: 'linear-gradient(150deg, #0F4724 0%, #176C38 55%, #1E8B48 100%)' }}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <RumboSeal size={26} />
                <span className="text-[0.8125rem] font-extrabold tracking-[0.1em]">RUMBO</span>
              </div>
              <span className="text-[0.5625rem] font-bold uppercase tracking-[0.24em] text-white/70">
            ...
            <div className="flex flex-1 items-center gap-6 py-3">
              ...
              <dl className="flex flex-1 flex-col gap-2.5 min-w-0">
                {[
                  ...
                  <div key={label} className="min-w-0">
                    <dt className="text-[0.5625rem] font-bold uppercase tracking-[0.2em] text-white/55">{label}</dt>
                    <dd className={cn('text-[0.8125rem] font-semibold truncate', label === 'Folio' || label === 'Código' ? 'font-mono tracking-wider' : '')}>
                      {value}
                    </dd>
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center gap-2 flex-shrink-0">
                ...
                <span className="max-w-[8.125rem] text-center text-[0.5938rem] leading-snug text-white/75">
            ...
            <div className="border-t border-white/15 pt-2.5 text-center">
              <span className="font-mono text-[0.5938rem] tracking-[0.1em] text-white/60 break-all">{verifyUrl}</span>
            </div>
      ...
      {/* Hint girar */}
      <p className="mt-2.5 flex items-center justify-center gap-1.5 text-[0.6875rem] text-[var(--ink-muted)] select-none">
