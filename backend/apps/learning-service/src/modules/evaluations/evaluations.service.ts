...
export class EvaluationsService {
  ...
  create(dto: CreateEvaluationDto, actor: string) {
    return this.prisma.evaluation.create({
      data: { title: dto.title, description: dto.description ?? null, courseId: dto.courseId, createdBy: actor, updatedBy: actor },
    });
  }
  ...
  async submitAttempt(dto: SubmitAttemptDto) {
    ...
    return this.prisma.evaluationAttempt.create({
      data: { evaluationId: dto.evaluationId, studentId: dto.studentId, answers: dto.answers as any, createdBy: dto.studentId },
    });
  }
}
